...
<nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 shadow-lg" id="bottom-nav">
    <div class="max-w-4xl mx-auto flex items-center justify-around py-2">
        ...
        <a href="{{ route('matches') }}" class="flex flex-col items-center gap-0.5 px-3 py-1 rounded-lg transition {{ request()->routeIs('matches*') ? 'text-heart-red' : 'text-gray-400 hover:text-brown' }}">
        ...
        {{-- Planes o Likes (condicional segun suscripcion) --}}
        @php
            $hasActiveSubscription = Auth::user()->activeSubscription !== null;
        @endphp

        @if($hasActiveSubscription)
            {{-- Boton Likes con menu desplegable --}}
            <div x-data="{ likesOpen: false }" class="relative">
                ...
                <div x-show="likesOpen" @click.away="likesOpen = false"
                     ...
                     class="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50"
                     style="display: none;">
