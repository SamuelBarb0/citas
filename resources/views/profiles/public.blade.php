...
<div class="min-h-screen bg-gradient-to-br from-cream via-white to-cream">
    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="sticky bottom-20 z-40 bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg px-4 py-3 border border-gray-200">
                <div class="flex gap-3">
                    <!-- Botón de Like -->
                    <form action="{{ route('like.store', $user->id) }}" method="POST" class="flex-1">
                        @csrf
                        <input type="hidden" name="liked_user_id" value="{{ $user->id }}">
                        <button type="submit" class="w-full bg-gradient-to-r from-heart-red to-heart-red-light text-white py-3 px-4 rounded-xl hover:shadow-glow transition-all font-bold text-sm flex items-center justify-center gap-2 group">
                    ...
                    @if($hasMatch)
                        ...
                        @endphp
                        <a href="{{ route('messages.show', $match->id) }}" class="flex-1 bg-white text-brown border-2 border-brown py-3 px-4 rounded-xl hover:bg-brown hover:text-white transition-all font-bold text-sm flex items-center justify-center gap-2 group">
                    ...
                    @else
                        <div class="flex-1 bg-gray-100 text-gray-400 py-3 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 cursor-not-allowed">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
