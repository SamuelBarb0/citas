                                ...
                                <a href="{{ route('messages.show', $msg->match_id) }}" class="flex items-center gap-3 p-2 rounded-xl hover:bg-cream transition">
                                    <div class="relative flex-shrink-0">
                                        ...
                                        @if($msg->unreadCount > 0)
                                            <span class="absolute -top-1 -right-1 bg-heart-red text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">{{ $msg->unreadCount }}</span>
                                        @endif
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="font-semibold text-brown text-sm truncate">{{ $msg->otherProfile->nombre }}</p>
                                        <p class="text-gray-500 text-xs truncate">{{ Str::limit($msg->mensaje, 35) }}</p>
                                    </div>
                                    <span class="text-[10px] text-gray-400 flex-shrink-0">{{ $msg->created_at->diffForHumans(null, true) }}</span>
                                </a>
                            @endif
                        @endforeach
                    </div>
                        ...
                        @foreach($matches as $match)
                            @if($match->otherProfile)
                                <a href="{{ route('profile.public', $match->otherProfile->user_id) }}" class="flex-shrink-0 text-center group">
                                    <div class="w-16 h-16 rounded-full overflow-hidden border-2 border-heart-red/20 group-hover:border-heart-red transition shadow-sm">
                                    ...
                                    <p class="text-xs font-semibold text-brown mt-1 truncate w-16">{{ $match->otherProfile->nombre }}</p>
                                </a>
                            @endif
                        @endforeach
                    </div>
